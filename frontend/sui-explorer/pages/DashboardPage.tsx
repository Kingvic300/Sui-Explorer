...
const ProjectsPage: React.FC = () => {
    ...
    const recentlyViewedProjects = useMemo(() => {
        return recentlyViewed.map(id => allProjects.find(p => p.id === id)).filter(Boolean) as Project[];
        ...
        <Card className="!p-0">
            ...
            <div className="p-4 space-y-3 min-h-[18rem]">
               ...
               {!isLoadingTransactions && transactions && transactions.length > 0 && (
                   transactions.map(tx => <TransactionListItem key={tx.id} tx={tx} onClick={() => setSelectedTx(tx)} />)
               )}
               {!isLoadingTransactions && (!transactions || transactions.length === 0) && (
