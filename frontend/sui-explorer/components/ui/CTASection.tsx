...
const GatewayVisual = () => (
    <div className="relative w-full h-80 lg:h-full flex items-center justify-center [perspective:800px]">
        <div className="absolute w-full h-full animate-gateway-rotate" style={{ transformStyle: 'preserve-3d' }}>
            {/* Rings - making them more visible in light mode */}
            <div className="absolute inset-10 border border-blue-500/50 dark:border-blue-500/30 rounded-full animate-spin-slow"></div>
            <div className="absolute inset-20 border border-indigo-500/50 dark:border-indigo-500/30 rounded-full animate-spin-slow-reverse"></div>
            <div className="absolute inset-32 border-2 border-blue-500/70 dark:border-blue-500/50 rounded-full animate-spin-slow"></div>
            
            {/* Glows - making them more visible in light mode */}
            <div className="absolute w-1/2 h-1/2 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-blue-500/40 dark:bg-blue-500/30 rounded-full blur-3xl animate-pulse"></div>
            <div className="absolute w-1/3 h-1/3 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-indigo-400/50 dark:bg-indigo-400/40 rounded-full blur-2xl animate-pulse" style={{ animationDelay: '0.5s' }}></div>

            {/* Central Orb - adding a visible shadow in light mode */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-8 h-8 bg-gradient-to-br from-white to-blue-300 rounded-full shadow-2xl shadow-blue-400/40 dark:shadow-white/50"></div>
        </div>
    </div>
);
...
const CTASection: React.FC<CTASectionProps> = ({ onNavigateToDashboard, animatedBackgroundsEnabled }) => {
  return (
    <>
      <section className="relative py-20 lg:py-32 overflow-hidden bg-light-card dark:bg-dark-card/50 rounded-xl">
            ...
            <div className="grid lg:grid-cols-2 gap-16 items-center">
                <div>
                    ...
                    <AnimatedItem delay={150}>
                        <p className="text-lg text-slate-900 dark:text-slate-200 mb-8 max-w-lg">
