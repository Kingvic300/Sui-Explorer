...
const DetailRow: React.FC<{ label: string, value: string, isAddress?: boolean, onCopy?: (value: string) => void }> = ({ label, value, isAddress = false, onCopy }) => {
    ...
    return (
        <div className="flex justify-between items-center py-3 border-b border-light-border dark:border-dark-border/60">
            <span className="text-sm text-slate-600 dark:text-slate-400">{label}</span>
            <div className="flex items-center gap-2">
                <span className="font-mono text-sm text-slate-900 dark:text-slate-100 font-medium">{displayValue}</span>
                {onCopy && (
                     <button onClick={() => onCopy(value)} className="text-slate-700 dark:text-slate-500 hover:text-slate-900 dark:text-slate-100 transition-colors" aria-label={`Copy ${label}`}>
...
const TransactionDetailModal: React.FC<TransactionDetailModalProps> = ({ tx, onClose }) => {
    ...
    return (
        <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 transition-opacity duration-300 ${isClosing ? 'opacity-0' : 'opacity-100'}`}>
            <div className="absolute inset-0 bg-black/50 dark:bg-black/70 backdrop-blur-sm backdrop-blur-sm" onClick={handleClose}></div>
            <m.div
                ...
                className="modal-gradient-bg relative bg-white dark:bg-dark-bg w-full max-w-md rounded-2xl overflow-hidden shadow-2xl shadow-blue-500/10 border border-light-border dark:border-dark-border"
            >
                <div className="p-6">
                    <div className="flex justify-between items-start">
                        <h2 id="tx-modal-title" className="font-display text-2xl font-bold text-slate-900 dark:text-slate-100">Transaction Details</h2>
                        <button onClick={handleClose} aria-label="Close modal" className="p-2 bg-black/50 rounded-full text-slate-900 dark:text-slate-100 hover:bg-black/50 dark:bg-black/70 backdrop-blur-sm transition-colors">
                ...
                <div className="px-6 pb-6 flex flex-col items-center">
                    ...
                    <p className="text-slate-700 dark:text-slate-300 mt-1 capitalize">{tx.description}</p>
                </div>

                <div className="px-6 pb-6 bg-light-card dark:bg-dark-card border-t border-light-border dark:border-dark-border">
                 ...
                 <div className="p-4 bg-white dark:bg-dark-bg border-t border-light-border dark:border-dark-border">
